...
const ADDRESS_4: IAddress = { channelId: 'console',
    user: { id: 'userId4', name: 'user4' },
    bot: { id: 'bot', name: 'Bot' },
    conversation: { id: 'user4Conversation' }
};

function containsAddress(addresses: IAddress[], address: IAddress): boolean {
    return !!find(addresses, (curAddr: IAddress) => isEqual(curAddr, address));
}
...
export function providerTest(getNewProvider: () => Promise<IProvider>, providerName: string): void {
    ...
    const watchOnlyAgentAddress = ADDRESS_4;
    ...
    describe(providerName, () => {
        ...
        describe('customer queue for agent (customer goes into wait state)', () => {
            ...
            it('updates conversation state to be wait', () => {
                return provider.getConversationFromCustomerAddress(customer1Address)
                    .then((conversation: IConversation) => {
                        ...
                        expect(conversation.watchingAgents).to.be.empty;
                    });
            });
            ...
            it('agents can watch without affecting state', () => {
                return provider.watchConversation(customer1Address, agent1Address1)
                    .then((convo: IConversation) => {
                        expect(convo.watchingAgents.length).to.be.equal(1);
                        expect(convo.conversationState).to.be.equal(ConversationState.Wait);
                    });
            });
        });
        ...
        describe('customer connecting to agent', () => {
            ...
            it('agents can watch without affecting state', () => {
                return provider.watchConversation(customer1Address, agent1Address2)
                    .then((convo: IConversation) => {
                        expect(convo.watchingAgents.length).to.be.equal(2);
        ...
        xdescribe('agents watching conversation', () => {
            ...
            // TODO this should be changed to check for agent address added
            ...
            // TODO update this to check for inclusion in watching arr
            ...
            beforeEach(() => {
                return provider.addCustomerMessageToTranscript(customerMessage)
                    ...
                    .then((conversation: IConversation) => {
                        ...
                        expect(conversation.watchingAgents.length).to.be.equal(1);
